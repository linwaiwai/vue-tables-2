module.exports = function(h, that) {
    ...
    data.map(function(row, index) {
      ...
      if (that.opts.childRow)
       columns.push(<td class='is-center'><span on-click={that.toggleChildRow.bind(that,row[rowKey])} class={`VueTables__child-row-toggler ` + that.childRowTogglerClass(row[rowKey])}></span></td>);
      ...
      if (that.opts.childRow && this.rowsToggleState['row_' + row[rowKey]]) {
        ...
        rows.push(<tr class='VueTables__child-row'><td class='is-center' colspan={that.allColumns.length+1}>{template}</td></tr>);
      }
